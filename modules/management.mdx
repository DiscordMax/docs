...
Additionally, Report Bot supports multilingual communities, allowing server admins to set the bot’s language for clearer communication with users and staff.
## Language Configuration
Report Bot supports multiple languages to make moderation accessible for communities around the world. It automatically adapts to each user's Discord language settings—so users see commands, and their options in their own language without needing to configure anything manually.

Server managers can use the `/config language` command to set a default language for Report Bot’s internal configuration messages (like on embeds or messages). This does not override individual user preferences with command names and options, but helps standardize the bot’s responses.
